...
<td>66-page IREB requirements catalogue (LuaLaTeX compilation)</td>
</tr>
</table>
...
│
├── 📂 docs/                           # ── Documentation ──────────────────
│   ├── OTC_X_Anforderungskatalog_v31.tex     # IREB requirements catalogue (LaTeX)
│   ├── OTC_X_Anforderungskatalog_v31.pdf     # Compiled 66-page PDF (LuaLaTeX)
...
The `docs/` directory contains a **66-page IREB-compliant Requirements Engineering catalogue** written in LaTeX. The document is set with **LuaLaTeX + fontspec** (IBM Plex, Inter, JetBrains Mono). To compile the PDF locally, the following toolchain is required:

### Prerequisites

| Tool | Purpose | Download |
|:---|:---|:---|
| **MiKTeX** | LaTeX distribution (lualatex, makeindex) | [miktex.org](https://miktex.org/download) |

> **Fonts:** MiKTeX provides the required font packages (`plex`, `inter`, `jetbrainsmono-otf`) — install them via the MiKTeX console or `mpm --install=<pkg>`. If LuaLaTeX cannot resolve a font by name, refresh the font database once with `luaotfload-tool --update`.

### Compile the PDF

```bash
# From docs/ — three LuaLaTeX passes with the glossary built in between
lualatex OTC_X_Anforderungskatalog_v31.tex
makeindex -s OTC_X_Anforderungskatalog_v31.ist -o OTC_X_Anforderungskatalog_v31.acr OTC_X_Anforderungskatalog_v31.acn
lualatex OTC_X_Anforderungskatalog_v31.tex
lualatex OTC_X_Anforderungskatalog_v31.tex
del *.acn *.acr *.alg *.aux *.glo *.ist *.log *.toc *.lof *.lot *.out
```
...
| Command | Effect |
|:---|:---|
| `lualatex` (1st pass) | Typesets the document and writes the acronym list (`.acn`) plus TOC/LOF/LOT auxiliaries |
| `makeindex -s … .ist` | Compiles the acronym glossary (`.acn` → `.acr`) using the style file generated by the `glossaries` package — no Perl required, unlike `makeglossaries` |
| `lualatex` (passes 2–3) | Pull in the compiled glossary and settle all cross-references, bookmarks and page numbers |
| `del *.acn ...` | Removes the auxiliary intermediaries, leaving only `.tex` and `.pdf` |

The compiled PDF is written to `docs/OTC_X_Anforderungskatalog_v31.pdf`.
