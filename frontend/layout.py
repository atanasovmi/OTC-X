...
from frontend.components import render_kpis, render_market_table, render_native_dataframe
from frontend.formatting import fmt_chf, fmt_num, fmt_pct, pct_cls, score_badge

MARKET_FILTER_COLS = [2, 1, 1, 0.9]


def render_overview_tab(df_hist: pd.DataFrame, latest: pd.DataFrame) -> None:
    render_kpis(latest)
    ...
    with col_act:
        st.markdown(
            '<div class="sec-hdr">Market Activity — Last 90 Days</div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            chart_market_activity(df_hist), use_container_width=True, theme=None
        )
    with col_tree:
        st.markdown(
            '<div class="sec-hdr">Sector Allocation by Volume</div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            chart_sector_treemap(latest), use_container_width=True, theme=None
        )
    ...
    with col_mov:
        st.markdown(
            '<div class="sec-hdr">Top Movers — Price Change %</div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            chart_top_movers(latest), use_container_width=True, theme=None
        )
    with col_vol:
        st.markdown(
            '<div class="sec-hdr">Volume by Sector (CHF)</div>',
            unsafe_allow_html=True,
        )
        st.plotly_chart(
            chart_volume_by_sector(latest), use_container_width=True, theme=None
        )


def render_market_data_tab(df_hist: pd.DataFrame, latest: pd.DataFrame) -> None:
    st.markdown(
    ...
    n_display = (
        len(df_filt) if rows_to_show == "All" else min(int(rows_to_show), len(df_filt))
    )
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(
        '<div class="sec-hdr">Security Detail View</div>', unsafe_allow_html=True
    )
    st.markdown(
    ...
    if active_isins:
        ...
        st.plotly_chart(
            chart_security_history(df_hist, sel_isin),
            use_container_width=True,
            theme=None,
        )
...
def render_analytics_tab(df_hist: pd.DataFrame, latest: pd.DataFrame) -> None:
    st.markdown(
        '<div class="sec-hdr">Analysis Controls</div>', unsafe_allow_html=True
    )
    ...
    with col_sc:
        ...
        st.plotly_chart(
            chart_scatter_volume_price(analytics_df),
            use_container_width=True,
            theme=None,
        )
    with col_amh:
        ...
        st.plotly_chart(
            chart_amihud_by_sector(hist_filtered),
            use_container_width=True,
            theme=None,
        )
    ...
    top_vol_sectors = (
        vol_hist_data.groupby("Sektor")["trades_today"].sum().nlargest(5).index.tolist()
    )
    ...
    for idx, sec_name in enumerate(top_vol_sectors):
        with vol_btn_cols[idx]:
            ...
            if st.button(btn_label, key=f"vol_sec_{sec_name}", use_container_width=True):
    ...
    with d3c1:
        x_sel = st.selectbox(
            "X-Axis", dim_keys, index=0, format_func=lambda k: dim_options[k], key="3d_x"
        )
    with d3c2:
        y_sel = st.selectbox(
            "Y-Axis", dim_keys, index=1, format_func=lambda k: dim_options[k], key="3d_y"
        )
    with d3c3:
        z_sel = st.selectbox(
            "Z-Axis", dim_keys, index=2, format_func=lambda k: dim_options[k], key="3d_z"
        )
    with d3c4:
        color_sel = st.selectbox(
            "Colour", dim_keys, index=5, format_func=lambda k: dim_options[k], key="3d_c"
        )
    with d3c5:
        size_sel = st.selectbox(
            "Size", dim_keys, index=4, format_func=lambda k: dim_options[k], key="3d_s"
        )
...
def render_anomaly_tab(df_hist: pd.DataFrame, latest: pd.DataFrame) -> None:
    total = len(latest)
    ...
    st.markdown(
        '<div class="sec-hdr">Risk Summary — Click to Filter</div>',
        unsafe_allow_html=True,
    )
    ...
    for col_idx, (lbl, val, border, tier_cls, scores) in enumerate(risk_tiers):
        with rc[col_idx]:
            ...
            active_style = (
                f"border: 2px solid {border}; box-shadow: 0 0 8px {border}40;"
                if is_active
                else ""
            )
    ...
    st.plotly_chart(
        chart_anomaly_severity_treemap(latest), use_container_width=True, theme=None
    )

    st.markdown("<br>", unsafe_allow_html=True)

    selected_tier = st.session_state.get("anomaly_tier", None)
    if selected_tier and selected_tier in config.SEVERITY_TIERS:
        tier_scores = config.SEVERITY_TIERS[selected_tier]
