...
from .config import (
    BRAND_RED, BRAND_DARK, GREEN_POS, RED_NEG, BORDER_COL, MUTED,
    PLOTLY_TPL, SECTOR_PALETTE, SEVERITY_TIERS
)
from .styling import hex_to_rgba
from .utils import fmt_pct, fmt_chf
...
def chart_market_activity(df_hist: pd.DataFrame) -> go.Figure:
    """Daily market activity with volume bars and trade count line."""
    daily = (
...
def chart_sector_treemap(latest: pd.DataFrame) -> go.Figure:
    """Sector volume treemap with average price change coloring."""
    g = (
...
def chart_top_movers(latest: pd.DataFrame, n: int = 14) -> go.Figure:
    """Horizontal bar chart of top price movers (gainers and losers)."""
...
def chart_volume_by_sector(latest: pd.DataFrame) -> go.Figure:
    """Horizontal bar chart of total volume by sector."""
    s = (
...
def chart_security_history(df_hist: pd.DataFrame, isin: str) -> go.Figure:
    """Historical price and volume chart for a single security with volatility band."""
